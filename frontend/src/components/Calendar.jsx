...
export default function Calendar({ title }) {
  ...
  const achieved = Object.keys(checkedDays).filter(
    key => checkedDays[key] && key.startsWith(`${current.year}-${current.month}-`)
  ).length
  ...
  function prevMonth() {
    if (current.year === MARCH_2026.year && current.month === MARCH_2026.month) return
    setCurrent(prev => {
  ...
  const isStart = current.month === MARCH_2026.month && current.year === MARCH_2026.year
